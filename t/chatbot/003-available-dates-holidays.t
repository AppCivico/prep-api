...
db_transaction {
    ...
    ok my $now = DateTime->now;
    ok my $dow = $now->day_of_week;
    ...
    subtest 'Chatbot | Get available dates' => sub {
        ...
        ok my $calendar_holidays    = $schema->resultset('CalendarHoliday')->next;
        ok my $content              = $calendar_holidays->get_content_decoded;
        ok $content->{manual_input} = [ $now->ymd ];
        ok $content                 = to_json($content);
        ok $calendar_holidays->update( { content => $content } );
        ok $calendar_holidays->discard_changes;
        ...
        ok grep { $_->{ymd} ne $now->ymd } @{$res->{dates}};
    };
