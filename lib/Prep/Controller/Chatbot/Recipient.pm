package Prep::Controller::Chatbot::Recipient;
use Mojo::Base 'Prep::Controller';

sub stasher {
    my $c = shift;

    $c->validate_request_params(
        fb_id => {
            type     => 'Num',
            required => 1,
        },
    );

    $c->stash( collection => $c->schema->resultset('Recipient') );

    my $recipient = $c->schema->resultset('Recipient')->search( { fb_id => $c->req->params->to_hash->{fb_id} } )->next;
    die \['fb_id', 'invalid'] unless $recipient;

    $c->stash(
        recipient  => $recipient,
        collection => $c->stash('collection')->search_rs( { id => $recipient->id } )
    )
}

sub post {
    my $c = shift;

    my $params = $c->req->params->to_hash;

    my $recipient = $c->schema->resultset('Recipient')->execute(
        $c,
        for  => 'create',
        with => $params
    );

    return $c
    ->redirect_to('current')
    ->render(
        json   => { id => $recipient->id },
        status => 201,
    );
}

sub get {
    my $c = shift;

    my $recipient = $c->stash('recipient');

    return $c->render(
        status => 200,
        json   => {

            id                            => $recipient->id,
            fb_id                         => $recipient->fb_id,
            name                          => $recipient->name,
            integration_token             => $recipient->integration_token,
            page_id                       => $recipient->page_id,
            picture                       => $recipient->picture,
            opt_in                        => $recipient->opt_in,
            finished_quiz                 => $recipient->finished_quiz,
            updated_at                    => $recipient->updated_at,
            created_at                    => $recipient->created_at,
            is_eligible_for_research      => $recipient->recipient_flag->is_eligible_for_research,
            is_part_of_research           => $recipient->recipient_flag->is_part_of_research,
            is_prep                       => $recipient->recipient_flag->is_prep,
            is_target_audience            => $recipient->recipient_flag->is_target_audience,
            finished_publico_interesse    => $recipient->recipient_flag->finished_publico_interesse,
            finished_recrutamento         => $recipient->recipient_flag->finished_recrutamento,
            finished_quiz_brincadeira     => $recipient->recipient_flag->finished_quiz_brincadeira,
            risk_group                    => $recipient->recipient_flag->risk_group,
            signed_term                   => $recipient->signed_term,
            has_appointments              => $recipient->has_appointments,
            prep_since                    => $recipient->recipient_flag->prep_since,
            city                          => $recipient->city,
            system_labels                 => $recipient->system_labels,
            phone                         => $recipient->phone,
            instagram                     => $recipient->instagram,
            voucher_type                  => $recipient->voucher_type,
            prep_reminder_before          => $recipient->prep_reminder_before,
            prep_reminder_before_interval => $recipient->prep_reminder_before_interval,
            prep_reminder_after           => $recipient->prep_reminder_after,
            prep_reminder_after_interval  => $recipient->prep_reminder_after_interval,
        }
    )
}

sub put {
    my $c = shift;

    my $params = $c->req->params->to_hash;

    my $recipient = $c->stash('recipient');

    $recipient->execute(
        $c,
        for  => 'update',
        with => $params
    );

    return $c->render(
        $c,
        code => 200,
        json => { id => $recipient->id }
    )
}

1;
